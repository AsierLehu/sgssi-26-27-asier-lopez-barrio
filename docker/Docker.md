...
**Respuesta:** Con `docker push usuario/nombre-imagen:tag`. Antes hay que iniciar sesión.
...
**Respuesta:** Un mensaje de que todo ha ido bien: el deamon se ha descargado la imagen, se ha creado el contenedor y se ha ejecutado el proceso.
...
**Respuesta:** De Docker Hub (imagen oficial). Si no está en el local, automaticamente al hacer `docker run` se descarga del repo remoto.

- ¿Qué diferencia hay entre `docker run` y `docker run -it`?

**Respuesta:** `-it` mantiene el input stdin abierto (`-i`) y crea una terminal(`-t`), permitiendo usar una terminal dentro del contenedor.

- ¿Por qué ha cambiado el prompt de la terminal?

**Respuesta:** Porque estamos dentro del contenedor.

- Si hacemos un listado mediante `ls`, ¿A qué maquina pertenecen los directorios?

**Respuesta:** Al sistema de ficheros del contenedor, no al de mi usuario.

- ¿Qué output nos da el comando `docker ps -a`?

**Respuesta:** Lista todos los contenedores (en ejecución y parados) y su información como ID, imagen y demás.
...
**Respuesta:** Parar (`docker kill`) detiene el proceso pero el contenedor sigue existiendo (sirve si hemos abierto una terminal dentro del contenedor). Borrar (`docker rm`) elimina el contenedor del sistema.

- ¿Cómo afecta a la imagen de la que ha surgido el contenedor?

**Respuesta:** No la modifica.

- ¿Cómo se borra una imagen?

**Respuesta:** Con `docker rmi nombre_o_id`.
...
**Respuesta:** `docker run` crea un contenedor nuevo. `docker exec` sirve para cuando tenemos abierto la terminal en un proceso para ejecutar un comando.
...
**Respuesta:** El contenido del fichero creado `/opt/msg`: que contiene el texto original de `msg` (p. ej. `Hola`) y, a continuación, la fecha/hora que se añadió con `RUN date >> /opt/msg`. 
...
**Respuesta:** El mensaje que en mi caso pone Hola, y luego la fecha y hora de creación.

- ¿Cómo cambiarías el mensaje que se obtiene?
**Respuesta:** Habria que modificar el archivo msg del ordenador y hacer build de la imagen otra vez.
...
**Respuesta:** Porque con `-v` se monta el mismo directorio del host (`dir-msg`) dentro del contenedor (`/app`), es un enlace compartido.
